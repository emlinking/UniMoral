...
def set_up_data_loader(text_path1: str, text_path2: str, text_path3: str, text_path4: str, set_type: str):
    ...
    if set_type == 'train':
        dataset1, dataset2, dataset3, dataset4 = process_dataset(dataset1, dataset2, dataset3, dataset4)
    ...
    if set_type == 'test':         ## No shuffling for test set
        return lab2idx1, lab2idx2, lab2idx3, lab2idx4, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, DataLoader(dataset1, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset2, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset3, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset4, batch_size=BATCH_SIZE, shuffle=False)
    return lab2idx1, lab2idx2, lab2idx3, lab2idx4, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, DataLoader(dataset1, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset2, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset3, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset4, batch_size=BATCH_SIZE, shuffle=True)

def process_dataset(dataset1, dataset2, dataset3, dataset4):
    max_len = max(len(dataset1['input_ids']), len(dataset2['input_ids']), len(dataset3['input_ids']), len(dataset4['input_ids']))
...
class MyLlamaModelMultiTaskDecide(nn.Module):
    def __init__(self, pretrained_model, num_inp1, num_inp2, num_inp3, num_inp4, num_classes_T1 = 2, num_classes_T2 = 2, num_classes_T3 = 2, num_classes_T4 = 2):
        super().__init__()
    ...
    def forward(self, source1=[], source2=[], source3=[], source4=[]):
        op1, op2, op3, op4 = [], [], [], []
        ...
        if type(op1) == torch.Tensor and type(op2) == torch.Tensor and type(op3) == torch.Tensor and type(op4) == torch.Tensor:
...
def prepare_for_training(model, learning_rate):
    ...
    return optimizer, criterion1, criterion2, criterion3, criterion4

def train_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, optimizer, criterion1, criterion2, criterion3, criterion4):
    model.train()
    epoch_train_loss = 0.0
    for step, (batch1, batch2, batch3, batch4) in enumerate(tqdm(zip(data_loader1, data_loader2, data_loader3, data_loader4), desc="Training Iteration")):
        ...
        logits1, logits2, logits3, logits4 = model(source1, source2, source3, source4)
...
def val_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, criterion1, criterion2, criterion3, criterion4):
    model.eval()
    epoch_val_loss = 0.0
    with torch.no_grad():
        for step, (batch1, batch2, batch3, batch4) in enumerate(tqdm(zip(data_loader1, data_loader2, data_loader3, data_loader4), desc="Validation Iteration")):
            input_ids, attention_mask, target1 = batch1
            source1 = {
                'input_ids': input_ids.to(DEVICE),
                'attention_mask': attention_mask.to(DEVICE)
...
def test_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, desc):
    model.eval()
    ...
    gc.collect()
    torch.cuda.empty_cache()
    return predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4

def compute_metrics(model, data_loader1, data_loader2, data_loader3, data_loader4, desc):

    predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4 = test_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, desc=desc)
    ...
    return predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4, result1, result2, result3, result4

def train(model, tokenizer, train_data_loader1, train_data_loader2, train_data_loader3, train_data_loader4, val_data_loader1, val_data_loader2, val_data_loader3, val_data_loader4, test_data_loader1, test_data_loader2, test_data_loader3, test_data_loader4, learning_rate, model_type):
    train_losses = []
    val_losses = []
    
    optimizer, criterion1, criterion2, criterion3, criterion4 = prepare_for_training(model=model, learning_rate=learning_rate)
    
    min_val_loss = 99999
    bad_loss = 0
    for epoch in range(MAX_EPOCHS):
        train_loss = train_epoch(model, train_data_loader1, train_data_loader2, train_data_loader3, train_data_loader4, optimizer, criterion1, criterion2, criterion3, criterion4)
        train_losses.append(train_loss)
        print("Epoch: {}\ttrain_loss: {}".format(epoch+1, train_loss))

        val_loss = val_epoch(model, val_data_loader1, val_data_loader2, val_data_loader3, val_data_loader4, criterion1, criterion2, criterion3, criterion4)
        val_losses.append(val_loss)

        if val_loss < min_val_loss:
            bad_loss = 0
            min_val_loss = val_loss
            test_pred1, test_gold1, test_pred2, test_gold2, test_pred3, test_gold3, test_pred4, test_gold4, test_results1, test_results2, test_results3, test_results4 = compute_metrics(model, tokenizer, test_data_loader1, test_data_loader2, test_data_loader3, test_data_loader4, desc="Test Iteration")
            ...
            path = OUTPUT_DIR + DATASET1 + "_" + DATASET2 + "_" + DATASET3 + "_" + DATASET4 + "_" + LOAD_MODEL
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_pathT1', default='/shared/0/Morality/OnlyFiles/ethics/commonsense/', type=str, help='The path containing train/val/test.csv for data 1')
    parser.add_argument('--file_pathT2', default='/shared/0/Morality/OnlyFiles/ethics/deontology/', type=str, help='The path containing train/val/test.csv for data 2')
    parser.add_argument('--file_pathT3', default='/shared/0/Morality/OnlyFiles/ethics/justice/', type=str, help='The path containing train/val/test.csv for data 3')
    parser.add_argument('--file_pathT4', default='/shared/0/Morality/OnlyFiles/moralexceptqa/', type=str, help='The path containing train/val/test.csv for data 4')
    ...
    lab2idx1, lab2idx2, lab2idx3, lab2idx4, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, train_dataset1, train_dataset2, train_dataset3, train_dataset4 = set_up_data_loader(
    ...
    _1, _2, _3, _4, __1, __2, __3, __4, val_dataset1, val_dataset2, val_dataset3, val_dataset4 = set_up_data_loader(
    ...
    _1, _2, _3, _4, __1, __2, __3, __4, test_dataset1, test_dataset2, test_dataset3, test_dataset4 = set_up_data_loader(
        text_path1 = TEXT_INPUT_PATH_T1 + '/test.csv',
        text_path2 = TEXT_INPUT_PATH_T2 + '/test.csv',
        text_path3 = TEXT_INPUT_PATH_T3 + '/test.csv',
    ...
    print(f"Train datasets: {len(train_dataset1)}, {len(train_dataset2)}, {len(train_dataset3)}, {len(train_dataset4)}")
    print(f"Val datasets: {len(val_dataset1)}, {len(val_dataset2)}, {len(val_dataset3)}, {len(val_dataset4)}")
    print(f"Test datasets: {len(test_dataset1)}, {len(test_dataset2)}, {len(test_dataset3)}, {len(test_dataset4)}")
    # ------------------------------ MODEL SETUP ------------------------------ #
