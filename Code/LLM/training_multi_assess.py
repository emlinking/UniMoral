...
def set_up_data_loader(text_path1: str, text_path2: str, text_path3: str, text_path4: str, text_path5: str, set_type: str):
    ...
    dataset4, lab2idx4, NUM_INP4 = preprocess_dataset(text_path=text_path4)
    dataset5, lab2idx5, NUM_INP5 = preprocess_dataset(text_path=text_path5)

    if set_type == 'train':
        dataset1, dataset2, dataset3, dataset4, dataset5 = process_dataset(dataset1, dataset2, dataset3, dataset4, dataset5)
    ...
    dataset4 = TensorDataset(dataset4['input_ids'], dataset4['attention_mask'], dataset4['labels'])
    dataset5 = TensorDataset(dataset5['input_ids'], dataset5['attention_mask'], dataset5['labels'])

    if set_type == 'test':         ## No shuffling for test set
        return lab2idx1, lab2idx2, lab2idx3, lab2idx4, lab2idx5, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, NUM_INP5, DataLoader(dataset1, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset2, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset3, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset4, batch_size=BATCH_SIZE, shuffle=False), DataLoader(dataset5, batch_size=BATCH_SIZE, shuffle=False)
    return lab2idx1, lab2idx2, lab2idx3, lab2idx4, lab2idx5, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, NUM_INP5, DataLoader(dataset1, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset2, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset3, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset4, batch_size=BATCH_SIZE, shuffle=True), DataLoader(dataset5, batch_size=BATCH_SIZE, shuffle=True)

def process_dataset(dataset1, dataset2, dataset3, dataset4, dataset5):
    max_len = max(len(dataset1['input_ids']), len(dataset2['input_ids']), len(dataset3['input_ids']), len(dataset4['input_ids']), len(dataset5['input_ids']))
    ...
    if len(dataset4['input_ids']) < max_len: 
        diff = max_len - len(dataset4['input_ids'])
        new_idx = np.random.choice(list(range(len(dataset4['input_ids']))), size=diff, replace=True)
        new_inp_id = []
        new_attn_mask = []
        new_target = []
        for i in new_idx:
            new_inp_id.append(dataset4['input_ids'][i].unsqueeze(0))
            new_attn_mask.append(dataset4['attention_mask'][i].unsqueeze(0))
            new_target.append(dataset4['labels'][i].unsqueeze(0))
        new_inp_id = torch.cat(new_inp_id, 0)
        new_attn_mask = torch.cat(new_attn_mask, 0)
        new_target = torch.cat(new_target, 0)

        dataset4['input_ids'] = torch.cat((dataset4['input_ids'], new_inp_id), 0)
        dataset4['attention_mask'] = torch.cat((dataset4['attention_mask'], new_attn_mask), 0)
        dataset4['labels'] = torch.cat((dataset4['labels'], new_target), 0)

    if len(dataset5['input_ids']) < max_len: 
        diff = max_len - len(dataset5['input_ids'])
        new_idx = np.random.choice(list(range(len(dataset5['input_ids']))), size=diff, replace=True)
        new_inp_id = []
        new_attn_mask = []
        new_target = []
        for i in new_idx:
            new_inp_id.append(dataset5['input_ids'][i].unsqueeze(0))
            new_attn_mask.append(dataset5['attention_mask'][i].unsqueeze(0))
            new_target.append(dataset5['labels'][i].unsqueeze(0))
        new_inp_id = torch.cat(new_inp_id, 0)
        new_attn_mask = torch.cat(new_attn_mask, 0)
        new_target = torch.cat(new_target, 0)

        dataset5['input_ids'] = torch.cat((dataset5['input_ids'], new_inp_id), 0)
        dataset5['attention_mask'] = torch.cat((dataset5['attention_mask'], new_attn_mask), 0)
        dataset5['labels'] = torch.cat((dataset5['labels'], new_target), 0)
...
class MyLlamaModelMultiTaskDecide(nn.Module):
    def __init__(self, pretrained_model, num_inp1, num_inp2, num_inp3, num_inp4, num_inp5, num_classes_T1 = 2, num_classes_T2 = 2, num_classes_T3 = 2, num_classes_T4 = 2, num_classes_T5 = 2):
        super().__init__()
        ...
        self.task4_classify = nn.Linear(num_inp4*768, num_classes_T4)

        self.task5_head = nn.Sequential(
            nn.LayerNorm(4096),
            nn.Linear(4096, 768), 
            nn.LayerNorm(768),
            nn.Dropout(p=0.1, inplace=False)
        )
        self.task5_classify = nn.Linear(num_inp5*768, num_classes_T5)
        
    def forward(self, source1=[], source2=[], source3=[], source4=[], source5=[]):
        op1, op2, op3, op4, op5 = [], [], [], [], []
        ...
        if source4:
            if DATASET4 in input_label_sets:
                op = self.encoder(source4['input_ids'], source4['attention_mask']).last_hidden_state.mean(dim=1)
                op = self.task4_head(op)
                op4 = self.task4_classify(op)

            elif DATASET4 in inputs_label_sets:
                op = []
                source4['input_ids'] = torch.transpose(source4['input_ids'], 0, 1)
                source4['attention_mask'] = torch.transpose(source4['attention_mask'], 0, 1)
                for input_ids, attention_mask in zip(source4['input_ids'], source4['attention_mask']):
                    op.append(self.task4_head(self.encoder(input_ids, attention_mask).last_hidden_state.mean(dim=1)))
                op = torch.cat(op, dim = 1)
                op4 = self.task4_classify(op)

            elif DATASET4 in input_labels_sets:
                op = self.encoder(source4['input_ids'], source4['attention_mask']).last_hidden_state.mean(dim=1)
                op = self.task4_head(op)
                op4 = self.task4_classify(op)

            for item in source4.items():
                del item

        if source5:
            if DATASET5 in input_label_sets:
                op = self.encoder(source5['input_ids'], source5['attention_mask']).last_hidden_state.mean(dim=1)
                op = self.task5_head(op)
                op5 = self.task5_classify(op)

            elif DATASET5 in inputs_label_sets:
                op = []
                source5['input_ids'] = torch.transpose(source5['input_ids'], 0, 1)
                source5['attention_mask'] = torch.transpose(source5['attention_mask'], 0, 1)
                for input_ids, attention_mask in zip(source5['input_ids'], source5['attention_mask']):
                    op.append(self.task5_head(self.encoder(input_ids, attention_mask).last_hidden_state.mean(dim=1)))
                op = torch.cat(op, dim = 1)
                op5 = self.task5_classify(op)

            elif DATASET5 in input_labels_sets:
                op = self.encoder(source5['input_ids'], source5['attention_mask']).last_hidden_state.mean(dim=1)
                op = self.task5_head(op)
                op5 = self.task5_classify(op)

            for item in source5.items():
                del item
        ...
        if type(op1) == torch.Tensor and type(op2) == torch.Tensor and type(op3) == torch.Tensor and type(op4) == torch.Tensor and type(op5) == torch.Tensor:
            ...
            return op3
        elif type(op4) == torch.Tensor:
            return op4
        elif type(op5) == torch.Tensor:
            return op5
...
def prepare_for_training(model, learning_rate):
    ...
    return optimizer, criterion1, criterion2, criterion3, criterion4, criterion5

def train_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, data_loader5, optimizer, criterion1, criterion2, criterion3, criterion4, criterion5):
    model.train()
    epoch_train_loss = 0.0
    for step, (batch1, batch2, batch3, batch4, batch5) in enumerate(tqdm(zip(data_loader1, data_loader2, data_loader3, data_loader4, data_loader5), desc="Training Iteration")):
        ...
        input_ids, attention_mask, target4 = batch4
        source4 = {
            'input_ids': input_ids.to(DEVICE),
            'attention_mask': attention_mask.to(DEVICE)
        }

        input_ids, attention_mask, target5 = batch5
        source5 = {
            'input_ids': input_ids.to(DEVICE),
            'attention_mask': attention_mask.to(DEVICE)
        }
        ...
        logits1, logits2, logits3, logits4, logits5 = model(source1, source2, source3, source4, source5)
        
        loss1 = criterion1(logits1, target1)
        loss2 = criterion2(logits2, target2)
        loss3 = criterion3(logits3, target3)
        loss4 = criterion4(logits4, target4)
        loss5 = criterion5(logits5, target5)
...
def val_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, data_loader5, criterion1, criterion2, criterion3, criterion4, criterion5):
    model.eval()
    epoch_val_loss = 0.0
    with torch.no_grad():
        for step, (batch1, batch2, batch3, batch4, batch5) in enumerate(tqdm(zip(data_loader1, data_loader2, data_loader3, data_loader4, data_loader5), desc="Validation Iteration")):
            input_ids, attention_mask, target1 = batch1
            source1 = {
                'input_ids': input_ids.to(DEVICE),
                'attention_mask': attention_mask.to(DEVICE)
...
def test_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, data_loader5, desc):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for step, batch4 in enumerate(tqdm(data_loader4, desc=desc)):
            input_ids, attention_mask, target4 = batch4
            source4 = {
                'input_ids': input_ids.to(DEVICE),
                'attention_mask': attention_mask.to(DEVICE)
            ...
            target4 = target4.to(DEVICE)
            logits4 = model(source4 = source4)

            predicted_classes4 = torch.argmax(nn.Softmax(dim=1)(logits4), dim=1)
            predictions4.extend(predicted_classes4.tolist())
            gold4.extend(target4.tolist())
            ...
            for item in source4.items():
                del item
            del source4
            del predicted_classes4

        for step, batch5 in enumerate(tqdm(data_loader5, desc=desc)):
            input_ids, attention_mask, target5 = batch5
            source5 = {
                'input_ids': input_ids.to(DEVICE),
                'attention_mask': attention_mask.to(DEVICE)
            ...
            target5 = target5.to(DEVICE)
            logits5 = model(source5 = source5)

            predicted_classes5 = torch.argmax(nn.Softmax(dim=1)(logits5), dim=1)
            predictions5.extend(predicted_classes5.tolist())
            gold5.extend(target5.tolist())
    ...
    gc.collect()
    torch.cuda.empty_cache()
    return predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4, predictions5, gold5

def compute_metrics(model, data_loader1, data_loader2, data_loader3, data_loader4, data_loader5, desc):

    predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4, predictions5, gold5 = test_epoch(model, data_loader1, data_loader2, data_loader3, data_loader4, data_loader5, desc=desc)
    result1 = get_scores(gold1, predictions1)
    result2 = get_scores(gold2, predictions2)
    result3 = get_scores(gold3, predictions3)
    result4 = get_scores(gold4, predictions4)
    result5 = get_scores(gold5, predictions5)

    torch.cuda.empty_cache() 
    
    return predictions1, gold1, predictions2, gold2, predictions3, gold3, predictions4, gold4, predictions5, gold5, result1, result2, result3, result4, result5

def train(model, tokenizer, train_data_loader1, train_data_loader2, train_data_loader3, train_data_loader4, train_data_loader5, val_data_loader1, val_data_loader2, val_data_loader3, val_data_loader4, val_data_loader5, test_data_loader1, test_data_loader2, test_data_loader3, test_data_loader4, test_data_loader5, learning_rate, model_type):
    train_losses = []
    val_losses = []
    
    optimizer, criterion1, criterion2, criterion3, criterion4, criterion5 = prepare_for_training(model=model, learning_rate=learning_rate)
    
    min_val_loss = 99999
    bad_loss = 0
    for epoch in range(MAX_EPOCHS):
        train_loss = train_epoch(model, train_data_loader1, train_data_loader2, train_data_loader3, train_data_loader4, train_data_loader5, optimizer, criterion1, criterion2, criterion3, criterion4, criterion5)
        train_losses.append(train_loss)
        print("Epoch: {}\ttrain_loss: {}".format(epoch+1, train_loss))

        val_loss = val_epoch(model, val_data_loader1, val_data_loader2, val_data_loader3, val_data_loader4, val_data_loader5, criterion1, criterion2, criterion3, criterion4, criterion5)
        val_losses.append(val_loss)

        if val_loss < min_val_loss:
            bad_loss = 0
            min_val_loss = val_loss
            test_pred1, test_gold1, test_pred2, test_gold2, test_pred3, test_gold3, test_pred4, test_gold4, test_pred5, test_gold5, test_results1, test_results2, test_results3, test_results4, test_results5 = compute_metrics(model, tokenizer, test_data_loader1, test_data_loader2, test_data_loader3, test_data_loader4, test_data_loader5, desc="Test Iteration")
            ...
            print("\nTest Accuracy for DATASET ", DATASET4, ": ", test_results4['accuracy'])
            print("\nTest Classification Report for DATASET ", DATASET4, ":\n", test_results4['report'])

            print("\nTest Accuracy for DATASET ", DATASET5, ": ", test_results5['accuracy'])
            print("\nTest Classification Report for DATASET ", DATASET5, ":\n", test_results5['report'])
        
            path = OUTPUT_DIR + DATASET1 + "_" + DATASET2 + "_" + DATASET3 + "_" + DATASET4 + "_" + DATASET5 + "_" + LOAD_MODEL
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_pathT1', default='/shared/0/Morality/OnlyFiles/anecdotes/', type=str, help='The path containing train/val/test.csv for data 1')
    parser.add_argument('--file_pathT2', default='/shared/0/Morality/OnlyFiles/ethics/virtue/', type=str, help='The path containing train/val/test.csv for data 2')
    parser.add_argument('--file_pathT3', default='/shared/0/Morality/OnlyFiles/moralstories/consequence_classification/', type=str, help='The path containing train/val/test.csv for data 3')
    parser.add_argument('--file_pathT4', default='/shared/0/Morality/OnlyFiles/storal/mocpt_data/storal_en/', type=str, help='The path containing train/val/test.csv for data 4')
    parser.add_argument('--file_pathT5', default='/shared/0/Morality/OnlyFiles/storal/mopref_data/storal_en/', type=str, help='The path containing train/val/test.csv for data 5')
    parser.add_argument('--batch_size', default=2, type=int, help='The batch size for training')
    ...
    TEXT_INPUT_PATH_T4 = "/".join(args.file_pathT4.split("/")[:-1])
    TEXT_INPUT_PATH_T5 = "/".join(args.file_pathT5.split("/")[:-1])

    DATASET1 = args.file_pathT1.split("/")[-2]
    DATASET2 = args.file_pathT2.split("/")[-2]
    DATASET3 = args.file_pathT3.split("/")[-2]
    DATASET4 = args.file_pathT4.split("/")[-2]
    DATASET5 = args.file_pathT5.split("/")[-2]
    ...
    lab2idx1, lab2idx2, lab2idx3, lab2idx4, lab2idx5, NUM_INP1, NUM_INP2, NUM_INP3, NUM_INP4, NUM_INP5, train_dataset1, train_dataset2, train_dataset3, train_dataset4, train_dataset5 = set_up_data_loader(
        text_path1 = TEXT_INPUT_PATH_T1 + '/train.csv',
        text_path2 = TEXT_INPUT_PATH_T2 + '/train.csv',
        text_path3 = TEXT_INPUT_PATH_T3 + '/train.csv',
        text_path4 = TEXT_INPUT_PATH_T4 + '/train.csv',
        text_path5 = TEXT_INPUT_PATH_T5 + '/train.csv',
        set_type = 'train'
    )

    _1, _2, _3, _4, _5, __1, __2, __3, __4, __5, val_dataset1, val_dataset2, val_dataset3, val_dataset4, val_dataset5 = set_up_data_loader(
        text_path1 = TEXT_INPUT_PATH_T1 + '/val.csv',
        text_path2 = TEXT_INPUT_PATH_T2 + '/test.csv',
        text_path3 = TEXT_INPUT_PATH_T3 + '/val.csv',
        text_path4 = TEXT_INPUT_PATH_T4 + '/val.csv',
        text_path5 = TEXT_INPUT_PATH_T5 + '/val.csv',
        set_type = 'val'
    )

    _1, _2, _3, _4, _5, __1, __2, __3, __4, __5, test_dataset1, test_dataset2, test_dataset3, test_dataset4, test_dataset5 = set_up_data_loader(
        text_path1 = TEXT_INPUT_PATH_T1 + '/test.csv',
        text_path2 = TEXT_INPUT_PATH_T2 + '/test.csv',
        text_path3 = TEXT_INPUT_PATH_T3 + '/test.csv',
        text_path4 = TEXT_INPUT_PATH_T4 + '/test.csv',
        text_path5 = TEXT_INPUT_PATH_T5 + '/test.csv',
        set_type = 'test'
    )

    print(f"Train datasets: {len(train_dataset1)}, {len(train_dataset2)}, {len(train_dataset3)}, {len(train_dataset4)}, {len(train_dataset5)}")
    print(f"Val datasets: {len(val_dataset1)}, {len(val_dataset2)}, {len(val_dataset3)}, {len(val_dataset4)}, {len(val_dataset5)}")
    print(f"Test datasets: {len(test_dataset1)}, {len(test_dataset2)}, {len(test_dataset3)}, {len(test_dataset4)}, {len(test_dataset5)}")
    # ------------------------------ MODEL SETUP ------------------------------ #
    ...
    if args.model_type == 'llama_3':
        MODEL = MyLlamaModelMultiTaskDecide(
            ...
            num_classes_T4 = len(lab2idx4.keys()),
            num_classes_T5 = len(lab2idx5.keys())
        )
    ...
    train(model=MODEL,
        ...
        train_data_loader4=train_dataset4,
        train_data_loader5=train_dataset5,
        val_data_loader1=val_dataset1,
        val_data_loader2=val_dataset2,
        val_data_loader3=val_dataset3,
        val_data_loader4=val_dataset4,
        val_data_loader5=val_dataset5,
        test_data_loader1=test_dataset1,
        test_data_loader2=test_dataset2,
        test_data_loader3=test_dataset3,
        test_data_loader4=test_dataset4,
        test_data_loader5=test_dataset5,
